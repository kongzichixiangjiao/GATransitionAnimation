...
class LikeButtonMaskLayer: CAShapeLayer, CAAnimationDelegate {
    ...
    public func startAnimation() {
        ...
        let fromPath = UIBezierPath(arcCenter: CGPoint(x: self.bounds.width, y: self.bounds.height / 2), radius: 1, startAngle: 0, endAngle: CGFloat(M_PI) * 2.0, clockwise: false).cgPath
        let toPath = UIBezierPath(arcCenter: CGPoint(x: self.bounds.width, y: self.bounds.height / 2), radius: 15, startAngle: 0, endAngle: CGFloat(M_PI) * 2.0, clockwise: false).cgPath
        anim.delegate = self
        anim.duration = 0.2
